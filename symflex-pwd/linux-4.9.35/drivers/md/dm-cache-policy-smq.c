...
static int demote_cblock_resize(struct smq_policy *mq,dm_oblock_t *oblock)
{
	struct entry *demoted;

	printk(KERN_ALERT"Searching in clean queue\n");
	demoted = q_peek(&mq->clean, mq->clean.nr_levels, false);
	if (!demoted){
		/*
		 * We could get a block from mq->dirty, but that
	...
	printk(KERN_ALERT"calling locker function\n");

	del(mq, demoted);

	printk(KERN_ALERT"deleting block..\n");
	*oblock = demoted->oblock;
	free_entry(&mq->cache_alloc, demoted);
...
smq_do_resize(int count, int inflation, int *buf, struct policy_locker *locker, struct app_group_t *app_groups)
{
	struct entry *e;
	...
	if(inflation)
	{
		for(i=0; i<count; i++)
		{	
			if(allocator_empty(&mq1->cache_alloc)) 
			{
				r = demote_cblock_resize(mq1,&res.old_oblock);
				if(r == -1)
				{
					buf[i] = -1;
					return 0;
			...
			e = alloc_entry_resize(&mq1->cache_alloc);//gets an entry from free list.
			l_add_tail(mq1->cache_alloc.es, &mq1->cache_alloc.invalid, e);//add that entry to invalid list
			out  = infer_cblock(mq1, e);
			buf[i] = out;
			count1++;			
		}	
	...
	else
	{
		for(i=0; i<count; i++)
		{
			if(l_empty(&mq1->cache_alloc.invalid))
				return -1;

			e = get_entry(&mq1->cache_alloc, buf[i]);
			if(!e)
			{
			...
			l_del(mq1->cache_alloc.es, &mq1->cache_alloc.invalid, e);
			init_entry(e);
			l_add_tail(mq1->cache_alloc.es,&mq1->cache_alloc.free,e);
		}
...
//	struct bitmap_t *bitmap;
	...
	/*  if(register_vssd!=NULL){
	    printk(KERN_ALERT"calling hook\n");
	    excess = (*register_vssd)(100,1,buf);      
	    }
	 */
