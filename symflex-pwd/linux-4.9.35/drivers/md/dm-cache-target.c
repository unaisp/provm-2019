...
extern int send_message_to_backend(char *buf, int msg_size);
extern long int get_dm_IO_sectors(void);


DECLARE_DM_KCOPYD_THROTTLE_WITH_MODULE_PARM(cache_copy_throttle,
	"A percentage of time allocated for copying to and/or from cache");

extern unsigned long (*new_policy)(int count, int flag, int *buf, int round);
unsigned long do_resize(int count, int inflation, int *buf, int round);
extern unsigned long smq_do_resize(int count, int inflation, int *buf, struct policy_locker *locker, struct app_group_t *app_groups);
...
struct cache {
	...
	struct app_group_t app_groups[3];
...
static void load_stats(struct cache *cache)
{
	...
	/*	unaisp	*/
	atomic_set(&cache->stats.request_count, stats.request_count);
...
static void save_stats(struct cache *cache)
{
	...
	/*	unaisp	*/
	stats.request_count = atomic_read(&cache->stats.request_count);
...
static void migration_failure(struct dm_cache_migration *mg)
{
	...
	if (mg->writeback) {
		...
		cell_defer(cache, mg->new_ocell, true);
		printk("Old block : %llu.  New block: %llu.  Cache block: %u \n", mg->old_oblock, mg->new_oblock, mg->cblock);
	}
...
static int cell_locker_resize(struct policy_locker *locker, dm_oblock_t b)
{
	struct old_oblock_lock *l = container_of(locker, struct old_oblock_lock, locker);
	struct dm_bio_prison_cell *cell = NULL;
	struct dm_bio_prison_cell *cell_output = NULL;

	return bio_detain(l->cache, b, NULL, cell, 
			  (cell_free_fn) free_prison_cell,
			  l->cache, &cell_output);


	//	Return 		1 block under migration
	//				0 Not under migration
}
			 ...
			 struct dm_bio_prison_cell *new_ocell)
{
	...
	case POLICY_REPLACE:
		// printk(KERN_ALERT "dmn-cache:-----process_cell POLICY_REPLACE \n");
...
#define DEFAULT_MIGRATION_THRESHOLD 20480

/*********** SymFlex ***********/
struct task_struct *status_thread;
struct cache* cache_symflex;
static int status_thread_function(void *arg)
{
	long int a[100];
	long int sectors_start;
	long int sectors_end;
	long int rqst_start;
	long int rqst_end;
	long int miss_start;
	long int miss_end;
	...
	sectors_start = get_dm_IO_sectors();
	rqst_start = atomic_read(&cache_symflex->stats.request_count);
	miss_start = atomic_read(&cache_symflex->stats.read_miss) + atomic_read(&cache_symflex->stats.write_miss);

	while(1)
	{
		msleep(30 * 1000);

		sectors_end = get_dm_IO_sectors();
		rqst_end = atomic_read(&cache_symflex->stats.request_count);
		miss_end = atomic_read(&cache_symflex->stats.read_miss) + atomic_read(&cache_symflex->stats.write_miss);
		resident = from_cblock(policy_residency(cache_symflex->policy));
		cache_size = from_cblock(cache_symflex->cache_size - policy_invalid_blocks(cache_symflex->policy));
		// free_blocks = from_cblock(cache->cache_size - policy_invalid_blocks(cache->policy) - policy_residency(cache_symflex->policy));
	
		// hit_ratio = 1 - ((miss_end - miss_start) / (rqst_end - rqst_start));

		a[0] = sectors_end - sectors_start;		// # of sectors read
		a[1] = rqst_end - rqst_start;			// # Requests
		a[2] = miss_end - miss_start;			// # cache misses
		a[3] = resident;						// # Resident size
		a[4] = cache_size;

		printk("status: %ld. %ld. %ld. %ld  %ld \n", a[0], a[1], a[2], a[3], a[4]);

		send_message_to_backend((char *)a, 5 * sizeof(long int));
		printk(KERN_ALERT "Sending message: \n");
...
static int setup_statistics_thread(struct cache *c)
{
	status_thread = kthread_run(status_thread_function, NULL, "dm-cache status thread");
	printk(KERN_ALERT "Status thread created \n");
...
/*********** SymFlex ***********/
/*------------------------------RESIZE----------------------------------*/
struct cache* cache_global;

unsigned long do_resize(int count, int inflation, int *buf, int round)
{
	struct old_oblock_lock ool;

	ool.locker.fn = cell_locker_resize;
	ool.cache = cache_global;
	ool.structs = NULL;
	ool.cell = NULL;
	
	// struct policy_locker locker;
	// locker.fn = cell_locker;

	printk(KERN_ALERT "VSSD: do_resize \n");

	return smq_do_resize(count, inflation, buf, &ool.locker, cache_global->app_groups);
}
...
static int cache_ctr(struct dm_target *ti, unsigned argc, char **argv)
{
	...
	/*Symflex */
	cache_global = cache;
	new_policy = &do_resize; //actual resize
	...
	cache->app_groups[2].id = 2;

	cache->app_groups[0].allocated = 0;
	...
	printk("Cache ctr success. going to destroy args \n");
out:
	destroy_cache_args(ca);
	printk("Args are destroyed \n");
	return r;
}
...
static int cache_map(struct dm_target *ti, struct bio *bio)
{
	...
	switch (lookup_result.op) {
	case POLICY_HIT:
		// printk(KERN_ALERT "dmn-cache::-----cache map POLICY_HIT \n");
			 ...
			 unsigned status_flags, char *result, unsigned maxlen)
{
	...
	case STATUSTYPE_INFO:
		...
		DMEMIT("%u %llu/%llu %llu %llu/%llu/%llu %u %u %u %u %u %u %u %lu",
		       ...
		       (unsigned long long) from_cblock(cache->cache_size - invalid),
