				...
				gfp_t gfp)
{
	...
	if (unlikely(vq->broken)) {
		END_USE(vq);
		printk(KERN_ALERT "RING-ERROR: vq broken \n");
		return -EIO;
	}
	...
	// printk("RING-STATUS: vq: %p. len: %i. avail: %i \n", vq, descs_used, vq->vq.num_free);
	...
	if (vq->vq.num_free < descs_used) {
		...
		if (indirect)
			kfree(desc);
		END_USE(vq);
		printk(KERN_ALERT "RING-ERROR: Can't add buf len %i - avail = %i\n", descs_used, vq->vq.num_free);
		return -ENOSPC;
	}
...
unmap_release:
	printk(KERN_ALERT "RING-ERROR: unmap_release \n");
	err_idx = i;
	i = head;
