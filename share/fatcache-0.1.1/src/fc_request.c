...
req_process_set(struct context *ctx, struct conn *conn, struct msg *msg)
{
    ...
    ret =  itemx_removex(msg->hash, msg->md);
    if(ret)
        printf("An item with same key already exists. Its deleted \n");
    else
        printf("An item with same key does not exists \n");
