...
static inline unsigned long policy_do_resize(struct dm_cache_policy *p, int count, int inflation, int *buf, struct policy_locker *locker, struct app_group_t *app_groups)
{
	return p->do_resize(p, count, inflation, buf, locker, app_groups);
}
