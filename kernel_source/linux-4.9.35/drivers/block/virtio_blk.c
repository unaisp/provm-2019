...
long int interval(struct timeval start, struct timeval end)
{
		...
		const struct blk_mq_queue_data *bd)
{
	...
	memcpy(&vbr->times.rs, &t0, sizeof(struct timeval));

	BUG_ON(req->nr_phys_segments + 2 > vblk->sg_elems);

	/* Added by Bhavesh Singh. 2017.06.17. Begin add */

	/*static unsigned long long int request_count=0;
	unsigned long long int count=++request_count;
	...
	int bio_count = 0;
	unsigned long int physical_address, virtual_address;
	struct bio *tmp_bio = bio;*/

	/*for_each_bio(tmp_bio) 
	{
		printk(KERN_ALERT"\t\t Bio count = %d.  Number vectors in this bio : %u \n", ++bio_count, tmp_bio->bi_vcnt);

		struct bio_vec *bio_vec_temp = tmp_bio->bi_io_vec; 
		int bv_count=0;

		while(bv_count < tmp_bio->bi_vcnt)
		{
			physical_address = page_to_phys(bio_vec_temp->bv_page);
			virtual_address = phys_to_virt(physical_address);
			printk(KERN_ALERT" Vector = %d. Page = %x. len = %d, offset = %d. PhysicalAddress = %lld,  Virtual = %d \n ", 
				bv_count++,
				bio_vec_temp->bv_page, 
				bio_vec_temp->bv_len, 
				bio_vec_temp->bv_offset,
				physical_address,
				virtual_address);

			printk(KERN_ALERT" Contents = [%s] \n", virtual_address);
	...
	/*for_each_bio(bio) 
	{
		//printk(KERN_ALERT"virtio: Request count=%u, Bio count=%d \n", count, ++bio_count); 

		bio_for_each_segment(bvec, bio, iter) 
		{		
			sector_num = iter.bi_sector;
			//printk(KERN_ALERT"virtio: Request count=%u, Bio count=%d, Sector count=%d \n", count, bio_count, sector_num); 
...
static int init_vq(struct virtio_blk *vblk)
{
	...
	printk(KERN_INFO"VBLK: init_vq \n");

	err = virtio_cread_feature(vdev, VIRTIO_BLK_F_MQ,
			struct virtio_blk_config, num_queues,
			&num_vqs);
	printk(KERN_INFO"VBLK: num_queues = %d \n ", num_vqs);
	if (err)
		num_vqs = 1;
...
static int virtblk_probe(struct virtio_device *vdev)
{
	...
	printk(KERN_INFO"VBLK: \tNumber of vqs = %d \n", vblk->num_vqs);
	printk(KERN_INFO"VBLK : nr_hw_queues = %u \n", vblk->tag_set.nr_hw_queues);
