...
itemx_init(void)
{
    ...
    printf("Initalizing itemx \n");
    printf("\titemx table size = nitx_table = %lu.  \n",nitx_table );
    printf("\tsizeof 'struct itemx_tqh' = %lu. sizeo of 'struct itemx' = %lu \n", sizeof(struct itemx_tqh), sizeof(struct itemx));
    printf("\tMax index memory = %lu. Max Number if itemx = %lu \n", settings.max_index_memory, n);
